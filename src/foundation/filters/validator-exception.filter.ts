...
export class ValidationExceptionFilter implements ExceptionFilter {
   ...
   */
    ...
    const errorMsg = this.formatErrors(exception);

    // Create a new BadRequestException with the validation error message.
    const err = BadRequestException.VALIDATION_ERROR(errorMsg);
    const responseBody = {
      error: err.code,
      message: err.message,
  ...
  private formatErrors(error: ValidationError): string {
    ...
    if (error.children && error.children.length > 0) {
      return error.children.map(child => this.formatErrors(child)).join(', ');
    }
